<template>
    <v-card outlined>
        <v-card-title>
            CustomerSupportTicket # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/customerSupportTickets/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
